...
import {
    ...
    deleteProfessorsThunkCreator, deleteSubjectThunkCreator,
...
function SubjectItem(props) {
    ...
    const handleOkEdit = () => {
        ...
        const isDuplicate = subjects.some(subject => subject.name.toLowerCase() === name.toLowerCase());
        if(name !== '') {
            if (isDuplicate) {
                warning('Такой предмет уже существует');
            } else {
                dispatch(editSubjectThunkCreator(props.id, name, object.accessToken)).then(() => {
                    success("Предмет изменен");
    ...
    const deleteProfessor = () => {
        let object = JSON.parse (localStorage.getItem ("data"));
        dispatch(deleteSubjectThunkCreator(props.id, object.accessToken)).then(() => {
